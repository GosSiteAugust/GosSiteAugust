import React from "react";
import './main.css'
import CarouselComponent from "../../Carousel/Carousel";
import Services from "../../Services/Services";
import Footer from "../../Footer/Footer";
import UpdateBanner from "../../Updater/Updater";
import { useEffect, useState } from 'react';
import BtnCarouselComponent from "../../BtnsCarousel/BtnCarousel";
const Main = () => {
    const [isMobile, setIsMobile] = useState(false);
    useEffect(() => {
        const userAgent = navigator.userAgent.toLowerCase();
        setIsMobile(/android|webos|iphone|ipad|ipod|blackberry|iemobile|opera mini/i.test(userAgent));
      }, []);
    const [showBanner, setShowBanner] = useState(true);

    return (
        <>
        {isMobile && showBanner && <UpdateBanner onClose={() => setShowBanner(false)} />}
            <div className="header">
                <div className="logo">
                    <svg width="124" height="22" viewBox="0 0 124 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M71.7494 16.384H68.3074C68.1923 16.384 68.1086 16.2723 68.14 16.1606C68.7154 14.202 69.3152 11.985 69.7302 9.52718C70.1557 6.98553 70.3789 4.64289 70.473 2.55161C70.4765 2.45735 70.5532 2.38403 70.6474 2.38403H80.8765C80.9729 2.38403 81.051 2.46219 81.051 2.5586V16.2095C81.051 16.3059 80.9729 16.384 80.8765 16.384H77.5249C77.4284 16.384 77.3503 16.3059 77.3503 16.2095V5.66583H73.9858C73.8621 7.22183 73.6536 8.89377 73.3117 10.6583C72.9142 12.7252 72.4434 14.6 71.9133 16.2653C71.8924 16.3352 71.8226 16.384 71.7494 16.384Z" fill="#EE2F53"></path>
                        <path d="M0 2.5586C0 2.46219 0.0781548 2.38403 0.174564 2.38403H10.0567C10.1801 2.38403 10.2636 2.51116 10.2157 2.62608L8.9919 5.55922C8.95958 5.63669 8.88566 5.67194 8.81548 5.66496C8.80973 5.66554 8.8039 5.66583 8.798 5.66583H3.70075V16.2095C3.70075 16.3059 3.62259 16.384 3.52618 16.384H0.174564C0.0781548 16.384 0 16.3059 0 16.2095V2.5586Z" fill="#0066B3"></path>
                        <path d="M99.3222 2.5586C99.3222 2.46219 99.4004 2.38403 99.4968 2.38403H109.379C109.502 2.38403 109.586 2.51116 109.538 2.62608L108.314 5.55922C108.282 5.63669 108.208 5.67194 108.138 5.66496C108.132 5.66554 108.126 5.66583 108.12 5.66583H103.023V16.2095C103.023 16.3059 102.945 16.384 102.848 16.384H99.4968C99.4004 16.384 99.3222 16.3059 99.3222 16.2095V2.5586Z" fill="#EE2F53"></path>
                        <path d="M39.9967 2.38403C39.8756 2.38403 39.7914 2.5032 39.833 2.61571L44.9253 16.3863C44.2067 17.8971 43.4084 19.4873 42.4953 21.2599C42.4355 21.3759 42.5188 21.5162 42.6496 21.5162H46.2722C46.338 21.5162 46.3987 21.4785 46.4284 21.4199C49.109 15.9651 51.0816 11.1297 53.7785 2.61471C53.8153 2.50214 53.732 2.38403 53.6134 2.38403H50.2486C50.1705 2.38403 50.1015 2.4372 50.0802 2.51213C49.0418 6.15238 48.0845 9.0017 46.9748 11.7477L43.5539 2.49722C43.5288 2.42924 43.4634 2.38403 43.3903 2.38403H39.9967Z" fill="#EE2F53"></path>
                        <path d="M83.3189 2.38403C83.1978 2.38403 83.1136 2.5032 83.1552 2.61571L88.2475 16.3863C87.529 17.8971 86.7306 19.4873 85.8175 21.2599C85.7577 21.3759 85.841 21.5162 85.9718 21.5162H89.5944C89.6602 21.5162 89.7209 21.4785 89.7506 21.4199C92.4312 15.9651 94.4038 11.1297 97.1008 2.61471C97.1375 2.50214 97.0542 2.38403 96.9356 2.38403H93.5708C93.4927 2.38403 93.4237 2.4372 93.4024 2.51213C92.364 6.15238 91.4067 9.0017 90.297 11.7477L86.8761 2.49722C86.851 2.42924 86.7856 2.38403 86.7125 2.38403H83.3189Z" fill="#EE2F53"></path>
                        <path d="M65.5505 13.0804C65.5332 13.0355 65.4986 12.9976 65.4535 12.9769C65.4085 12.9562 65.3565 12.9562 65.3115 12.97C64.5078 13.2667 63.0008 13.5081 61.9511 13.5081C59.6542 13.5081 58.5594 12.8906 58.5594 9.38232C58.5594 6.57773 58.8955 5.2565 61.9511 5.2565C62.8172 5.2565 63.6209 5.38069 64.6429 5.67736C64.726 5.70151 64.8161 5.66356 64.8542 5.58767C65.2353 4.87359 65.696 4.00082 66.2157 2.93141C66.24 2.88312 66.24 2.82792 66.2192 2.77963C66.1984 2.73133 66.1568 2.69339 66.1048 2.67614C64.7607 2.24838 63.1567 2 61.712 2C56.9 2 54.7556 4.25954 54.7556 9.33058C54.7556 14.4741 56.9 16.7681 61.712 16.7681C62.9176 16.7681 65.2942 16.5163 66.4409 16.0609C66.531 16.023 66.5795 15.9195 66.5448 15.8298L65.5505 13.0804Z" fill="#EE2F53"></path>
                        <path d="M37.395 13.0804C37.3776 13.0355 37.343 12.9976 37.298 12.9769C37.2529 12.9562 37.201 12.9562 37.1559 12.97C36.3522 13.2667 34.8452 13.5081 33.7955 13.5081C31.4986 13.5081 30.4039 12.8906 30.4039 9.38232C30.4039 6.57773 30.7399 5.2565 33.7955 5.2565C34.6616 5.2565 35.4653 5.38069 36.4873 5.67736C36.5704 5.70151 36.6605 5.66356 36.6986 5.58767C37.0797 4.87359 37.5405 4.00082 38.0601 2.93141C38.0844 2.88312 38.0844 2.82792 38.0636 2.77963C38.0428 2.73133 38.0012 2.69339 37.9493 2.67614C36.6051 2.24838 35.0011 2 33.5564 2C28.7444 2 26.6 4.25954 26.6 9.33058C26.6 14.4741 28.7444 16.7681 33.5564 16.7681C34.762 16.7681 37.1386 16.5163 38.2853 16.0609C38.3754 16.023 38.4239 15.9195 38.3892 15.8298L37.395 13.0804Z" fill="#0066B3"></path>
                        <path d="M17.5806 2C12.8762 2 11.0444 4.0549 11.0444 9.32997C11.0444 14.6853 12.8762 16.7681 17.5806 16.7681C22.3227 16.7681 24.1716 14.6853 24.1716 9.32997C24.1716 4.0549 22.3227 2 17.5806 2ZM17.5806 13.6945C15.4373 13.6945 14.7696 13.1014 14.7696 9.3823C14.7696 5.42251 15.4989 5.07014 17.5806 5.07014C19.7034 5.07014 20.4464 5.42251 20.4464 9.3823C20.4464 13.1014 19.7651 13.6945 17.5806 13.6945Z" fill="#0066B3"></path>
                        <path d="M123.41 2.38403H120.061C119.963 2.38403 119.886 2.46067 119.886 2.55822V13.1068C119.16 13.3088 118.435 13.4063 117.618 13.4063C115.587 13.4063 114.998 12.835 114.921 10.4522V2.55822C114.921 2.46067 114.844 2.38403 114.746 2.38403H111.397C111.299 2.38403 111.222 2.46067 111.222 2.55822V2.57912C111.222 2.57912 111.222 10.5184 111.226 10.5288C111.306 15.0123 112.932 16.7681 116.948 16.7681C119.293 16.7681 121.91 16.1724 123.463 15.5802C123.536 15.5558 123.581 15.4861 123.581 15.4095V2.57912V2.55822C123.585 2.46067 123.508 2.38403 123.41 2.38403Z" fill="#EE2F53"></path>
                    </svg>
                </div>
                <div className="list">
                    <ul>
                        <li>
                            <button>
                                <p>Услуги</p>
                            </button>
                        </li>
                        <li>
                            <button>
                                <p>Документы</p>
                            </button>
                        </li>
                        <li>
                            <button>
                                <p>Заявления</p>
                            </button>
                        </li>
                        <li>
                            <button>
                                <p>Платежи</p>
                            </button>
                        </li>
                        <li>
                            <button>
                                <p>Помощь</p>
                            </button>
                        </li>
                        <li>
                            <a title="Поиск с умным ассистентом" href="/">
                                <img alt="Поиск с умным ассистентом" className="search-icon" src="../../icon/search.svg" />
                            </a>
                        </li>
                        <li className="login">
                            <button>
                                <p>Войти</p>
                            </button>
                        </li>
                    </ul>
                </div>
            </div>
            <div className="welcome_banner">
                <div className="list">
                    <button>
                        <img src='../../icon/pop_person_popular.svg' alt="img"/>
                        <p>Популярные <br></br> услуги</p>
                    </button>
                    <button>
                        <img src='../../icon/health.svg' alt="img"/>
                        <p>Здоровье</p><br></br>
                    </button>
                    <button>
                        <img src='../../icon/references.svg' alt="img"/>
                        <p>Справки <br></br> Выписки</p>
                    </button>
                    <button>
                        <img src='../../icon/pensions.svg' alt="img"/>
                        <p>Пенсии <br></br> Пособия</p>
                    </button>
                    <button>
                        <img src='../../icon/transport.svg' alt="img"/>
                        <p>Транспорт <br></br> Права</p>
                    </button>
                    <button>
                        <img src='../../icon/family.svg' alt="img"/>
                        <p>Семья</p><br></br>
                    </button>
                    <button>
                        <img src='../../icon/education.svg' alt="img"/>
                        <p>Образование <br></br> Дети</p>
                    </button>
                    <button>
                        <img src='../../icon/passport.svg' alt="img"/>
                        <p>Регистрация <br></br> Паспорт</p>
                    </button>
                    <button>
                        <img src='../../icon/fines.svg' alt="img"/>
                        <p>Штрафы <br></br> Налоги</p>
                    </button>
                    <button>
                        <img src='../../icon/home.svg' alt="img"/>
                        <p>Земля <br></br> Дом</p>
                    </button>
                    <button>
                        <img src='../../icon/another.svg' alt="img"/>
                        <p>Прочее</p><br></br>
                    </button>
                    <button>
                        <img src='../../icon/pop_person_departments.svg' alt="img"/>
                        <p>Ведомства</p><br></br>
                    </button>
                </div>
                <div className="btns">
                    {isMobile ? <BtnCarouselComponent/> : <div className="btns_area">
                        <button>
                            <img src='../../icon/Alice.svg' alt="img"/>
                            <div className="btn_text">
                                Госуслуги и Алиса
                            </div>
                        </button>
                        <button>
                            <img src='../../icon/actions.svg' alt="img"/>
                            <div className="btn_text">
                                Служба по контракту
                            </div>
                        </button>
                        <button>
                            <img src='../../icon/pension.svg' alt="img"/>
                            <div className="btn_text">
                                Все соцвыплати теперь в СФР
                            </div>
                        </button>
                    </div>}
                    <div className="textInput">
                        <input placeholder="Введите запрос..."></input>
                        <button title="Отправить запрос" tabindex="27" aria-hidden="true" disabled=""><svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M7.28827 11.7073L4.54102 3.41895L25.7778 13.9994L4.54102 24.5811L7.28792 16.2929L13.6886 15.4871C14.438 15.3927 15.0001 14.7554 15.0001 14.0001C15.0001 13.2447 14.438 12.6074 13.6886 12.5131L7.28827 11.7073Z" fill="#0D4CD3"></path>
                            <path d="M4.54102 3.41895L4.93121 2.63576C4.61489 2.47817 4.23481 2.52633 3.9678 2.75785C3.70079 2.98937 3.59926 3.35878 3.71045 3.69424L4.54102 3.41895ZM7.28827 11.7073L6.45771 11.9826C6.5638 12.3027 6.8444 12.5333 7.17898 12.5755L7.28827 11.7073ZM25.7778 13.9994L26.1681 14.7826C26.4651 14.6346 26.6529 14.3313 26.6528 13.9994C26.6528 13.6676 26.4651 13.3643 26.168 13.2163L25.7778 13.9994ZM4.54102 24.5811L3.71044 24.3058C3.59926 24.6413 3.7008 25.0107 3.96782 25.2422C4.23485 25.4737 4.61492 25.5219 4.93124 25.3643L4.54102 24.5811ZM7.28792 16.2929L7.17862 15.4247C6.84404 15.4669 6.56344 15.6975 6.45735 16.0176L7.28792 16.2929ZM13.6886 15.4871L13.5793 14.6189V14.6189L13.6886 15.4871ZM13.6886 12.5131L13.7979 11.6449H13.7979L13.6886 12.5131ZM3.71045 3.69424L6.45771 11.9826L8.11883 11.432L5.37158 3.14365L3.71045 3.69424ZM26.168 13.2163L4.93121 2.63576L4.15082 4.20213L25.3877 14.7826L26.168 13.2163ZM4.93124 25.3643L26.1681 14.7826L25.3876 13.2163L4.15079 23.7979L4.93124 25.3643ZM6.45735 16.0176L3.71044 24.3058L5.37159 24.8564L8.11849 16.5682L6.45735 16.0176ZM13.5793 14.6189L7.17862 15.4247L7.39721 17.161L13.7979 16.3552L13.5793 14.6189ZM14.1251 14.0001C14.1251 14.3144 13.8912 14.5797 13.5793 14.6189L13.7979 16.3552C14.9848 16.2058 15.8751 15.1964 15.8751 14.0001H14.1251ZM13.5793 13.3812C13.8912 13.4205 14.1251 13.6857 14.1251 14.0001H15.8751C15.8751 12.8037 14.9849 11.7944 13.7979 11.6449L13.5793 13.3812ZM7.17898 12.5755L13.5793 13.3812L13.7979 11.6449L7.39757 10.8392L7.17898 12.5755Z" fill="#0D4CD3"></path>
                        </svg>
                        </button>
                    </div>
                </div>
                <div className="btns_textarea">
                    <button>
                        <p>Запись к врачу</p>
                    </button>
                    <button>
                        <p>Единое пособие</p>
                    </button>
                    <button>
                        <p>Запись в школу</p>
                    </button>
                    <button>
                        <p>Регистрация ТС</p>
                    </button>
                    <button>
                        <p>Загранпаспорт</p>
                    </button>
                </div>
            </div>
            <div className="carousel">
                <div className="header">
                    <h1>Новое на портале</h1>
                </div>
                <CarouselComponent/>
                <Services/>
            </div>
            <Footer></Footer>
            <script src="../../../public/scripts/flickity.pkgd.min.js"></script>
        </>
    )
}

export default Main